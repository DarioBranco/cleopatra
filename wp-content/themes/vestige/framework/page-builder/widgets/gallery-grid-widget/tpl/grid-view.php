					...
					<div class="row">
						...
						<?php while ($gallery_listings->have_posts()) : $gallery_listings->the_post();
							...
							<?php if (!empty($instance['listing_layout']['carousel_mode'])) { ?>
								<li class="item">
								<?php } else { ?>
								<li class="grid-item col-md-<?php echo esc_attr($grid_column); ?> col-sm-6 format-<?php echo esc_attr($post_format).' '; 
                                              if (!empty($term_slug)) {
                            ...
                            switch (get_post_format()) {
								...
								if ($instance['show_post_meta']) { ?><h4><?php the_title(); ?></h4><?php } ?>
							</li>
						<?php endwhile; ?>
						<?php if (!empty($instance['listing_layout']['carousel_mode'])) { ?>
						</ul>
