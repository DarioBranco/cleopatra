...
if (!function_exists('imic_enqueue_styles')) {
	function imic_enqueue_styles()
	{
		...
		wp_register_style('theme-colors', IMIC_THEME_PATH . '/assets/colors/' . $theme_color_scheme, array(), $theme_info->get('Version'), 'all');
		if ($event_switch != '0') {
		...
		$color = (isset($imic_options['theme_color_type'])) ? $imic_options['theme_color_type'][0] : '';
		if ($color == 0) {
