                ...
                <div class="col-md-<?php echo esc_attr($class); ?>" id="content-col">
                    <article class="single-artwork-content">
                        <?php if (have_posts()) : while (have_posts()) : the_post(); ?>
                                                ...
                                                <div id="artwork-images" class="flexslider format-gallery">
                                                    ...
                                                    <ul class="slides">
                                                        <?php foreach ($artwork_images as $artwork_image) {
                                                            $img_caption = wp_get_attachment_caption($artwork_image);
                                                            $img_title = get_the_title($artwork_image);
                                                            ...
                                                            <?php if (isset($imic_options['switch_lightbox']) && $imic_options['switch_lightbox'] == 0) {
                                                                $Lightbox_init = '<li class="format-image"><a class="media-box" href="' . esc_url($image_full[0]) . '" data-rel="prettyPhoto[grouped]" title="'.esc_attr($img_caption).'">';
                                                            } elseif (isset($imic_options['switch_lightbox']) && $imic_options['switch_lightbox'] == 1) {
                                                                $Lightbox_init = '<li class="format-image"><a href="' . esc_url($image_full[0]) . '" class="magnific-gallery-image media-box" title="'.esc_attr($img_caption).'">';
                                                            }
                                                            echo '' . $Lightbox_init; ?><img src="<?php echo esc_url($image[0]); ?>" alt="<?php echo esc_attr($img_title); ?>" title="<?php echo esc_attr($img_caption); ?>"></a>
														<?php if($img_caption){ ?>
															<div class="slide-image-caption"><?php echo esc_attr($img_caption); ?></div>
														<?php } ?>
													</li>
                                                        <?php } ?>
                                                    </ul>
                                                    ...
                                                    <div class="accordion-inner">
                                                        ...
                                                        if ($terms) {
                                                            foreach ($terms as $term) {
																...
																$artist_description = wpautop($term->description);
                                                                echo '' . do_shortcode($artist_description) . '<br /><br />';
                                                                $slug[] = $term->term_id;
                                                            }
                                                        } ?></div>
                                                </div>
                                ...
                                <!-- Pagination -->
                                <ul class="pager">
                                    <li class="pull-left"><?php echo next_post_link('%link', esc_html__('&larr; Previous', 'vestige'), false); ?></li>
                                    <li class="pull-right"><?php echo previous_post_link('%link', esc_html__('Next &rarr;', 'vestige'), false); ?></li>
                                </ul>
                            <?php
                        endwhile;
                    endif; ?>
