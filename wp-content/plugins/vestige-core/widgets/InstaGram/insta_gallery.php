...
class vestige_insta_gallery extends WP_Widget
{
	// constructor
	public function __construct()
	{
		$widget_ops = array('description' => __("Show Instagram images in a grid.", 'vestige-core'));
		parent::__construct(false, $name = __('Instagram Gallery', 'vestige-core'), $widget_ops);
	}
	...
	<p>
		<label for="<?php echo esc_attr($this->get_field_id('username')); ?>"><?php _e('Username', 'vestige-core'); ?></label>
		<input class="widefat" id="<?php echo esc_attr($this->get_field_id('username')); ?>" name="<?php echo esc_attr($this->get_field_name('username')); ?>" type="text" value="<?php echo esc_attr($username); ?>" />
	</p>
	<p>
		<label for="<?php echo esc_attr($this->get_field_id('tag')); ?>"><?php _e('Tag', 'vestige-core'); ?></label>
	...
	<p>
		...
		<input type="checkbox" value="1" id="<?php echo $this->get_field_id( 'display_profile' ); ?>" name="<?php echo $this->get_field_name( 'display_profile' ); ?>" <?php checked( $display_profile); ?>/> <?php _e( 'Display Profile?', 'vestige-core' ); ?>
		</label>
	</p>
	<p>
		<label for="<?php echo $this->get_field_id( 'display_biography' ); ?>">
		<input type="checkbox" value="1" id="<?php echo $this->get_field_id( 'display_biography' ); ?>" name="<?php echo $this->get_field_name( 'display_biography' ); ?>" <?php checked( $display_biography); ?>/> <?php _e( 'Display Biography?', 'vestige-core' ); ?>
        <div style="padding-bottom: 0" class="description"><?php _e('Enables displaying the biography. Only for users', 'vestige-core'); ?></div>
		</label>
	</p>
	<p>
		<label for="<?php echo esc_attr($this->get_field_id('items')); ?>"><?php _e('Number of items', 'vestige-core'); ?></label>
        ...
        <div style="padding-bottom: 0" class="description"><?php _e('Number of items to display. Up to 12 for users, up to 72 for tags', 'vestige-core'); ?></div>
	</p>
	<p>
		<label for="<?php echo esc_attr($this->get_field_id('items_per_row')); ?>"><?php _e('Items per row', 'vestige-core'); ?></label>
        ...
        <div style="padding-bottom: 0" class="description"><?php _e('Number of items that will be displayed for each row', 'vestige-core'); ?></div>
	</p>
	<p>
		<label for="<?php echo esc_attr($this->get_field_id('margin')); ?>"><?php _e('Margin between items', 'vestige-core'); ?></label>
        ...
        <div style="padding-bottom: 0" class="description"><?php _e('Margin (percentage) between items in gallery/igtv', 'vestige-core'); ?></div>
	</p>
	<p>
		<label for="<?php echo esc_attr($this->get_field_id('image_size')); ?>"><?php _e('Image Size', 'vestige-core'); ?></label>
        ...
        <div style="padding-bottom: 0" class="description"><?php _e('Scale of items to build gallery. Accepted values [150, 240, 320, 480, 640]. Does not apply to video previews.', 'vestige-core'); ?></div>
	</p>
<?php
}
// update widget
...
add_action('widgets_init', function () {
	register_widget('vestige_insta_gallery');
});
?>