...
var msgcanvas = "";
//alert(contextmsg);
function printcanvasmsg(msg, msg2) {
    if (!contextmsg) {
        return;
    }
...
if (topics.length > iaiml + 1) {
    for (var i = 0; i < topics.length - iaiml - 1; i++) {
        startopics[i] = topics[i + iaiml + 1];
    }
}
*/
...
var textparse = [],
    iparse = 0,
    inputword = [];
var nouttemplate = 300,
    iouttemplate = new Array(nouttemplate);
var oldmsg = "",
    msgprocess = "",
    msgmsg = "",
    thatmsg = "";
...
function processinputstar(text0) {
    ...
    for (i = 0; i < istar; i++) {
        ...
        test = 1;
        if (k > 0) {
            if (text.substr(0, k) != starpatt[i]) {
                test = 0;
            };
        }
        if (test == 1) {
            l = starpatt2[i].length;
            test = 1;
            p = text.length;
            if (l > 0) {
                if (text.substr(p - l, l) != starpatt2[i]) {
                    test = 0;
                };
            }
            if (test == 1) {
...
function processinput0(text0) {
            ...
            for (p = 0; p < iparse; p++) {
                ...
                if (testiword[n] < i) {
                    ...
                    if (testnword[n] == 1) {
                        ...
                        if (len < 1 + 0.1 * text0.length) {
                            testpattern[n] += 1;
                        }
                        if (len < 1) {
                            testpattern[n] += 1 + krandom;
                            if (thats[n - 1] == thatmsg) {
                                testpattern[n] += 1.2 + krandom;
                            } //auxtext+="!!"+thatmsg;}
                        }
                        if (patterns[n - 1] == text0) {
                            testpattern[n] += 1.2 + krandom * 8;
                        }
                        if (thats[n - 1] != thatmsg && thats[n - 1] != "") {
                            testpattern[n] -= 1.2;
                        }
                        if (topicprev != "") {
                            if (topics[n - 1] == topicprev) {
                                testpattern[n] += ktopic * 0.2 + krandom;
                            }
                        }
    ...
    if (j > 0) {
        ...
        if (Math.random() < 0.7 || randomize == 0) {
            testsay222 = 1;
        }
        oldmsg = msg;
        outmsg = msg;
...
function subvars() {
    ...
    for (var i = 0; i < nvars; i++) {
        if (varnames[i] != "") {
            msg += varnames[i] + "=" + varvalues[i] + crlf;
            j += 1;
            if (j > 200) {
                break;
            }
...
function subrandomize() {
    if (randomize == 0) {
        if (confirm("set randomize on ?")) {
            randomize = 1;
            setvar("myaimlrandomize", 1);
            savevars();
        }
    } else {
        if (confirm("set randomize off ?")) {
            randomize = 0;
            setvar("myaimlrandomize", 0);
            savevars();
        }
    }
}
...
var topicprev = "",
    ...
    ktopic = 0;
var text00 = "";
//In questa funzione dovrebbe essere eseguita la risposta, non la send!
function processinput(text0) {
     //25sal!!
    sendMyMsg(text0);

    //Devo capire come fare per inserire la chiamata all'observer della risposta cosi da mostrare quello 
    //che mi dice la base di conoscenza 
    // sendMyMsg("prova prova");

    return ""; //Mettendo questo return forzato la risposta è: Prova prova
    // processinput inizia qua
    // msgcanvas=text0;
    ...
    // return txt+mymsg; // processinput termina qua 
}
var nvars = 2000;
var vark = [],
    varnames = [],
...
function savevars() {
    ...
    if (localStorage) {
        localStorage.setItem("chatbot_chung_small_savevars", text);
    }
}
...
function loadvars() {
    var text = "";
    if (localStorage) {
        text = localStorage.getItem("chatbot_chung_small_savevars");
    }
    if (text != null && text != "") {
        var textline = [];
        textline = text.split("§§");
        for (var i = 0; i < nvars; i++) {
            ...
            if (textline[i]) {
                ...
                try {
                    vark[i] = eval(textline[i].substr(k + 2, 9));
                } catch (e) {
                    vark[i] = i;
                }
            }
...
function formatwiki(text0) {
    ...
    for (var n = 0; n < 5; n++) {
        ...
        if (text.toLowerCase().indexOf("refer to:") < 0) {
            break;
        }
    }
    if (text.toLowerCase().indexOf("refer to:") >= 0) {
...
function saytest22() {
    ...
    var forcewiki = 0;
    if (intext22.substr(0, 5).toLowerCase() == "wiki ") {
        forcewiki = 1;
        intext22 = intext22.substr(5, 99);
    }
    if (Math.random() < 0.065 * randomwiki + forcewiki) { //0.2*
        ...
        if (mywikitext != "searching..." && (word22.length >= 1 && word22.length <= 3)) {
            mywikitext = "searching...";
            wikikeysearch = intext22;
            subwiki();
            return;
        }
    }
